...
void eyeFinder::setup(int width, int height, int _targetWidth, int _targetHeight, int divisor){
	...
	previousImg.allocate((int) wdiv, (int) hdiv);
	diffImg.allocate((int) wdiv, (int) hdiv);
	currentImg.allocate((int) wdiv, (int) hdiv);
...
//------------------------------------------------------------------------------------
bool eyeFinder::update(ofxCvGrayscaleAdvanced & _currentImg, int threshold, int minBlobSize, int maxBlobSize, bool bAllArea = true){
	...
	if (bUseBlur) {
		currentImg.blur((int) blur);							//Great!!(also for subtraction!!!)
	}
...
void eyeFinder::getRotatedBoundingBox(int blobNum){
	...
	for( int i=0; i< contourFinder.blobs[blobNum].pts.size(); i++) {
		CvPoint pt;
		pt.x = (int) contourFinder.blobs[blobNum].pts[i].x;
		pt.y = (int) contourFinder.blobs[blobNum].pts[i].y;
		cvSeqPush( output, &pt );
	}
