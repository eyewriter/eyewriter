...
void pupilFinder::setup(int width, int height, float _magRatio){
	...
	currentImg.allocate(width * magRatio, height * magRatio);
	imgBeforeThreshold.allocate(width * magRatio, height * magRatio);
	
	
	ellipseCal.setup(width * magRatio, height * magRatio);
...
//----------------------------------------------------
bool pupilFinder::update(ofxCvGrayscaleAdvanced & _currentImg, float threshold, float minBlobSize, float maxBlobSize){	
