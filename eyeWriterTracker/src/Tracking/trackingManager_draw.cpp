...
void trackingManager::drawGlintFinder(int x, int y){
	...
	if (tracker.gFinder.bIsVerticalLED) tracker.gFinder.contourFinder.draw(x + tracker.gFinder.glintROI.x, y);
	else	tracker.gFinder.contourFinder.draw(x, y + tracker.gFinder.glintROI.y);
...
//--------------------------------------------------------------
void trackingManager::drawCircles(){
	...
	for (int i = 0; i < trail.size(); i++) {
		ofCircle(trail[i].x + ofGetWidth()/2, ofGetHeight()/2 + trail[i].y, 10);
	}
