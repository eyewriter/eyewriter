...
void drawingScene::setup(){
	...
	curveToggle.setup("straight", "curve", true, 10, 110, 130, 75);
	
	//buttons that should be shown when drawing
	nextLetter.setup("next letter", 10, 215, 130, 75);	
	nextStroke.setup("next stroke", 10, 300, 130, 60);
	undoPoint.setup("undo point", 10, 390, 130, 60);
	undo.setup("undo stroke", 10, 460, 130, 60);
	...
	drawableButtons.push_back(&curveToggle);
	...
	curveToggle.setMaxCounter(buttonCount);
...
void drawingScene::update(float mouseX, float mouseY){
	...
	if( bDrawing ){
		...
		if( curveToggle.update(mx, my) ){
			bCurve = !curveToggle.getState();
		}
		
		groups.back().setIsCurve(bCurve);
