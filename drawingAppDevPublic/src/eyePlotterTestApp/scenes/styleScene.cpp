...
void styleScene::update(float mouseX, float mouseY){
	...
	switch(state){
		...
		case(STYLE_STROKE):
			if (pickColor.update(mx,my))	drawColors = pickColor.getState();
			
			if (brush.update(mx,my))		updateStyle("brushId", 1, 0, renderer.numBrushes-1, showAll, true);
			if (strokeDown.update(mx,my))	updateStyle("strokeWeight", -1, 0, 10, showAll, false);
			if (strokeUp.update(mx,my))		updateStyle("strokeWeight", 1, 0, 10, showAll, false);
		break;
			
		case(STYLE_FILL):
			if (pickColor.update(mx,my))	drawColors = pickColor.getState();
			
			if (!groups[whichGroup].style.toggs["doFill"] && toggleFill.getState()) toggleFill.active=false;
			if (groups[whichGroup].style.toggs["doFill"] && !toggleFill.getState()) toggleFill.active=true;
			
			if (toggleFill.update(mx,my))	setToggle("doFill",toggleFill.getState(),showAll);
			
			if (!groups[whichGroup].style.toggs["doHoles"] && toggleHoles.getState()) toggleHoles.active=false;
			if (groups[whichGroup].style.toggs["doHoles"] && !toggleHoles.getState()) toggleHoles.active=true;
			
			if (toggleHoles.update(mx,my))	setToggle("doHoles",toggleHoles.getState(),showAll);
		break;
		...
		case(STYLE_SHADOW):
			...
			if (shadowLeft.update(mx,my))	updateStyle("dropShadowX",-3,-500,500,showAll,false);
			if (shadowRight.update(mx,my))	updateStyle("dropShadowX",3,-500,500,showAll,false);
			if (shadowUp.update(mx,my))		updateStyle("dropShadowY",-3,-500,500,showAll,false);
			if (shadowDown.update(mx,my))	updateStyle("dropShadowY",3,-500,500,showAll,false);
			
			if (!groups[whichGroup].style.toggs["dotShadow"] && lighting.getState()) lighting.active=false;
			if (groups[whichGroup].style.toggs["dotShadow"] && !lighting.getState()) lighting.active=true;
			
			if (lighting.update(mx,my))		setToggle("dotShadow",lighting.getState(),showAll);
			
			if (!groups[whichGroup].style.toggs["shadowHash"] && hashes.getState()) hashes.active=false;
			if (groups[whichGroup].style.toggs["shadowHash"] && !hashes.getState()) hashes.active=true;
			
			if (hashes.update(mx,my))		setToggle("shadowHash",hashes.getState(),showAll);
		...
		case(STYLE_OUTLINE):
			...
			if (outlineDown.update(mx,my))	updateStyle("outline",-1,0,500,showAll,false);
			if (outlineUp.update(mx,my))	updateStyle("outline",1,0,500,showAll,false);
	...
	if (drawColors) {
		...
		switch(state){
			case(STYLE_FILL):		updateColor("fillColor",col,showAll);		break;
			case(STYLE_STROKE):		updateColor("strokeColor",col,showAll);		break;
			case(STYLE_SHADOW):		updateColor("shadowColor",col,showAll);		break;
			case(STYLE_OUTLINE):	updateColor("outlineColor",col,showAll);	break;
			default: break;
		}
	} else	cPicker.outColor = -1;
...
void styleScene::setToggle(string ss, bool val, bool all){
	if (all){
		for ( int i = 0; i < groups.size(); i++ ){
			groups[i].style.toggs[ss] = val;
		}
	}else{
		groups[whichGroup].style.toggs[ss] = val;
	}
}
