			...
			if (curStyle.atts["outline"] > 0){
				...
				for (int s = 0; s < curGroup.strokes.size(); s++){
					...
					for (int p = 0; p < curStroke.pts.size() - 1; p++){
						...
						float step = 60.0/l + .001*curStyle.atts["outline"];
						
						for (float d = 0; d <= 1; d += step ){				
							float offset = curStyle.atts["outline"] + curStyle.atts["strokeWeight"];
							ofFill();
		...
		for(int i = 0; i < groups.size(); ++i ){
			...
			for (int s = 0; s < curGroup.strokes.size(); s++){
				...
				for(int p = 0; p < curStroke.pts.size()-1; p++){
					...
					if (i == 1) dot*=-1;
					(curStyle.toggs["dotShadow"]) ? ((dot < 0) ? ofSetColor(((curStyle.colors["shadowColor"] >> 16) & 0xff)*.4,
			...
			if( curStyle.toggs["doFill"] && curGroup.strokes.size() ){
				
				//printf("DO FILL FOOL!\n");
				
				bool bHoles = curStyle.toggs["doHoles"];
				if( bHoles ){
